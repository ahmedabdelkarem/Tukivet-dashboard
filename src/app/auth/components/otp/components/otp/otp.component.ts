import { ChangeDetectionStrategy, Component, ElementRef, HostBinding, Input, OnInit, QueryList, ViewChildren, isDevMode } from '@angular/core';
import { AbstractControl, ControlValueAccessor, FormArray, FormControl, ValidationErrors, Validator } from '@angular/forms';

function getFormArray(count: number): FormArray {
  const arr = [];
...
export class OtpComponent implements ControlValueAccessor, Validator   {
  classList: string = 'otp-default otp-medium';

  @Input()
  set count(count: number) {
    this.inputs = getFormArray(count);
    this.#count = count;
  }
 
  @Input()
  set status(status: string) {
    this.classList = status;
  }

  @Input()
  set size(size: string) {
    this.classList = `${this.classList + ' ' + size}`;
  }
  ...
  #count = 4;
  #scheduledFocus: any = null;

  inputs = getFormArray(this.#count);
  ...
  writeValue(value: string): void {
    ...
    this.inputs.setValue(new Array(this.#count).fill(''));
  }
  ...
  validate(control: AbstractControl<string, string>): ValidationErrors | null {
    if (!control.value || control.value.length < this.#count) {
      return {
  ...
  handlePaste(e: ClipboardEvent, idx: number) {
    ...
    const regex = new RegExp(`\\d{${this.#count}}`);
